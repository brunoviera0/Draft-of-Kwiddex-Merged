  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex items-center justify-between px-1">
        ...
        {image && <Button variant="outline" size="sm" onClick={() => fileRef.current?.click()}
          className="h-7 px-2 text-xs border-border text-foreground bg-background hover:bg-muted">Replace</Button>}
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="flex flex-col justify-end">
          <Button className="bg-blue-600 hover:bg-blue-700 text-white w-full" onClick={() => {
            if (!entry.imgQ || !entry.imgR) return;
            ...
            if (r) {
              ...
              const stats = [
                ...
                ["Border", r.border.toFixed(1)], ["Text/stats", r.textScore.toFixed(1)],
      ...
      <div className="flex items-center justify-between py-2">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="border-border text-foreground bg-background hover:bg-muted" onClick={()=>setCurrentIdx((i)=>Math.max(0,i-1))} disabled={currentIdx<=0}><ChevronLeft className="w-4 h-4" /></Button>
          <span className="text-sm text-muted-foreground">{currentIdx+1} / {entries.length}</span>
          <Button variant="outline" size="sm" className="border-border text-foreground bg-background hover:bg-muted" onClick={()=>setCurrentIdx((i)=>Math.min(entries.length-1,i+1))} disabled={currentIdx>=entries.length-1}><ChevronRight className="w-4 h-4" /></Button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{backgroundColor: "rgba(0,0,0,0.6)"}}>
          <div className="bg-background border border-border rounded-xl p-6 max-w-sm w-full mx-4 space-y-4 shadow-xl">
            ...
            <div className="flex gap-3 justify-end">
              <Button variant="outline" className="border-border text-foreground bg-background hover:bg-muted" onClick={()=>setShowDeleteModal(false)}>Cancel</Button>
