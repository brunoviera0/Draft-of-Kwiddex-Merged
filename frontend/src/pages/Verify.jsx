...
function VerifyPageContent() {
  ...
  const isOwnCertificate = isAuthenticated && certResult?.reviewer_id &&
    (certResult.reviewer_id === user?.id || certResult.reviewer_id === user?.sub || certResult.reviewer_id === user?.email);
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className={`text-lg font-semibold ${certResult.valid ? "text-green-800 dark:text-green-300" : "text-red-800 dark:text-red-300"}`}>
                              {certResult.valid ? "This document is certified using Kwiddex and has not been modified."
