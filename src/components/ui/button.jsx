...
const Button = React.forwardRef(
  ({ className, variant, size, asChild = false, motion = true, ripple, ...props }, ref) => {
    const Comp = asChild ? Slot : "button"
    const resolvedVariant = variant ?? "default"
    const enableRipple = ripple ?? resolvedVariant === "default"
    return (
      <Comp
        className={cn(
          motion && "motion-interactive motion-button",
          buttonVariants({ variant, size, className })
        )}
        data-variant={resolvedVariant}
        data-ripple={enableRipple ? "" : undefined}
        ref={ref}
        {...props}
      />
    )
